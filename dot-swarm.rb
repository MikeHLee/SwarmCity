class DotSwarm < Formula
  include Language::Python::Virtualenv

  desc "Markdown-native, environment-first agent orchestration protocol"
  homepage "https://github.com/oasis-main/dot_swarm"
  url "https://files.pythonhosted.org/packages/source/d/dot-swarm/dot_swarm-1.0.0.tar.gz"
  sha256 "0db6847386d96c201c1df7ac047894dc765327fbc4fb9c15dfb74ecb83d47658"
  license "MIT"
  head "https://github.com/oasis-main/dot_swarm.git", branch: "main"

  depends_on "python@3.12"
  depends_on "rust" => :build  # cryptography wheel build for unsupported platforms
  ...
  test do
    # Version check + .swarm/ scaffold round-trip
    assert_match "1.0.0", shell_output("#{bin}/swarm --version")

    mkdir testpath/"sandbox"
    cd testpath/"sandbox" do
      system bin/"swarm", "init", "--visible"
      system bin/"swarm", "add", "smoke test item"
      assert_predicate testpath/"sandbox/.swarm/queue.md", :exist?
      assert_predicate testpath/"sandbox/.swarm/claims", :directory?
    end
  end
end
